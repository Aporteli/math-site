...
function moduleText(module: WorkspaceModule, copy: HomeCopy) {
  ...
  const links = module.links
    .map((link) => item.links[link.id as keyof typeof item.links])
    .join(" ");
...
export function WorkspaceHub({ locale, copy, tools }: WorkspaceHubProps) {
  ...
  const lookup = LOOKUP_ROWS.filter((row) => {
    if (!q) return true;
    return normalize(
      `${copy.lookup.rows[row.id]} ${copy.lookup.kinds[row.kind]}`,
    ).includes(q);
  });
  ...
  const extras = q
    ? extraCatalogTools().filter((tool) =>
        normalize(
          `${tools[tool.id].title} ${tools[tool.id].description} ${tools[tool.id].badge}`,
        ).includes(q),
      )
    : [];
                    ...
                    {lookup.map((row) => (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-ink">
                          {copy.lookup.rows[row.id]}
                        </td>
                        <td className="px-4 py-3 text-body">
                          {copy.lookup.kinds[row.kind]}
                        </td>
