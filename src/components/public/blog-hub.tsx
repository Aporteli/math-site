...
function postMatches(
  ...
  return normalize(
    `${content.title} ${content.excerpt} ${tagText}`,
  ).includes(query);
}
      ...
      <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-6">
        ...
        <ul className="mt-4 flex flex-wrap gap-1.5">
          {post.tags.map((tag) => (
            <li
              key={tag}
              className="rounded-full bg-paper-deep px-2 py-0.5 text-xs font-medium text-muted"
            >
              {copy.tags[tag]}
            </li>
          ))}
...
function DownloadCard({
  ...
  return (
    <article className="flex h-full min-w-0 flex-col rounded-2xl border border-hairline bg-white p-5 shadow-sm transition-all hover:border-navy/30 hover:shadow-md sm:p-6">
      ...
      <p className="mt-4 text-xs font-semibold tracking-wide text-brass">
        {categoryLabel}
      </p>
      <h3 className="mt-1 break-words text-lg font-semibold leading-snug text-ink">
        {content.title}
      </h3>
      <p className="mt-2 min-h-0 flex-1 break-words text-sm leading-relaxed text-body">
        {content.description}
...
export function BlogHub({ locale, author, copy }: BlogHubProps) {
  ...
  const matchingPosts = BLOG_POSTS.filter(
    (post) =>
      postInFilter(post, filter) &&
      postMatches(post, copy.posts[post.id], copy.tags, normalizedQuery),
  ).sort((a, b) => b.publishedAt.localeCompare(a.publishedAt));

  const matchingFiles = RESOURCE_DOWNLOADS.filter((file) =>
    fileMatches(file, copy.files[file.id], normalizedQuery),
  );

  const featured =
    filter === "all" && !normalizedQuery
      ? matchingPosts.find((post) => post.featured)
      : undefined;
            ...
            <>
              ...
              {showArticles ? (
                <p className="text-sm text-muted" aria-live="polite">
                  {copy.hero.articleCount.replace(
                    "{count}",
                    String(visibleArticles.length),
                  )}
                </p>
              ) : null}
            </>
          ...
          {showDownloads && visibleFiles.length > 0 && (
            <section aria-labelledby="downloads-title">
              <SectionHeading
                ...
                description={copy.downloads.subtitle}
              />
