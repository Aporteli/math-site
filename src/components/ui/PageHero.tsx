...
export function PageHero({ icon: Icon, eyebrow, badge, title, description, aside, footer }: PageHeroProps) {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-hairline bg-white shadow-sm motion-safe:animate-fade-up">
      ...
      <div className={aside ? 'relative grid lg:grid-cols-[minmax(0,1.2fr)_minmax(16rem,0.8fr)]' : 'relative'}>
        <div className="p-6 sm:p-8 lg:p-10">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {eyebrow ? <p className="text-sm font-bold tracking-wide text-brass">{eyebrow}</p> : null}
            {badge ? (
        ...
        {aside ? (
          <div className="flex flex-col justify-center gap-3 border-t border-hairline bg-paper-deep/75 p-6 sm:p-8 lg:border-t-0 lg:border-l">
            {aside}
          </div>
        ) : null}
      </div>
...
export function SectionHeading({ id, title, description }: { id?: string; title: string; description?: string }) {
  return (
    <div className="flex items-start gap-4 border-b border-hairline pb-4">
      <span className="mt-1 hidden h-10 w-1 shrink-0 rounded-full bg-brass sm:block" aria-hidden="true" />
      <div className="min-w-0 max-w-2xl">
        <h2 id={id} className="text-2xl font-bold tracking-tight break-words text-ink sm:text-3xl">
          {title}
        </h2>
        {description ? <p className="mt-2 text-base leading-relaxed text-pretty text-body">{description}</p> : null}
      </div>
    </div>
  );
}
