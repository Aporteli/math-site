  ...
  return (
    <div
      ...
      className={`group relative flex flex-col items-center gap-1.5 p-1.5 rounded-2xl cursor-pointer transition-all shrink-0 ${
        isActive
          ? "bg-navy-tint ring-2 ring-navy shadow-xs"
          : "hover:bg-paper border border-hairline"
      }`}
    >
...
export function TeacherWhiteboard({ copy }: { copy: WhiteboardCopy }) {
  ...
  const savePreferencesImmediately = useCallback(
    (updates: Partial<{ tool: ToolId; color: string; width: number; isDark: boolean; zoomScale: number; pageIdx: number }>) => {
    ...
    (next: CanvasElement[]) => {
      ...
      if (isHydratedRef.current) {
        try {
          ...
          console.error("Failed to save canvas elements to localStorage:", e);
        }
      }
  ...
  useEffect(() => {
    function onClickOutside(e: MouseEvent) {
      ...
      if (penMenuRef.current && !penMenuRef.current.contains(target)) {
      ...
      if (pagesTrayRef.current && !pagesTrayRef.current.contains(target)) {
        ...
        if (!el.closest?.("[data-tray-trigger]")) {
    ...
    <div className="relative flex h-full w-full min-h-0 flex-col overflow-hidden rounded-2xl border border-hairline bg-white shadow-sm">
      ...
      <div className="relative z-30 shrink-0 border-b border-hairline bg-gradient-to-b from-paper/60 to-white overflow-x-auto thin-scrollbar touch-pan-x">
        <div className="flex w-max min-w-full items-center gap-1.5 px-3 py-2">
          ...
          {/* Select Tool */}
          <ToolButton title={copy.tools.select} active={activeTool === "select"} onClick={() => setAndSaveTool("select")}>
          ...
          <div ref={penMenuRef} className="relative flex shrink-0 items-center">
            ...
            >
              ...
              <button
                ...
                className={`flex items-center justify-center px-1.5 h-full rounded-r-xl transition-colors border-l ${
                  activeTool === "pen"
                    ? "border-white/20 hover:bg-navy-strong"
                    : "border-hairline hover:bg-paper"
                }`}
              >
                <ChevronDown className={`size-3 transition-transform duration-200 ${isPenMenuOpen ? "rotate-180" : ""}`} />
              </button>
            </div>

            {isPenMenuOpen && (
              <div className="absolute top-10 left-0 z-[120] w-56 rounded-2xl bg-white p-3 shadow-2xl border border-hairline animate-in fade-in zoom-in-95 duration-150">
                ...
                <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-hairline">
                  {STROKE_SIZES.map((size) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setAndSaveWidth(size);
                        setIsPenMenuOpen(false);
                      }}
                      className={`size-7 flex items-center justify-center rounded-xl transition-colors ${
                        strokeWidth === size
          ...
          <div ref={shapesMenuRef} className="relative flex shrink-0 items-center">
            ...
            >
              ...
              >
                <ChevronDown className={`size-3 transition-transform duration-200 ${isShapesMenuOpen ? "rotate-180" : ""}`} />
              </button>
            </div>

            {isShapesMenuOpen && (
              <div className="absolute top-10 left-0 z-[120] w-48 rounded-2xl bg-white p-2.5 shadow-2xl border border-hairline animate-in fade-in zoom-in-95 duration-150">
          ...
          <ToolButton title={copy.tools.eraser} active={activeTool === "eraser"} onClick={() => setAndSaveTool("eraser")}>
          ...
          <div ref={colorMenuRef} className="relative flex shrink-0 items-center">
            ...
            >
              ...
              />
              <ChevronDown className={`size-3 text-muted transition-transform duration-200 ${isColorMenuOpen ? "rotate-180" : ""}`} />
            </button>

            {isColorMenuOpen && (
              <div className="absolute top-10 left-0 z-[120] w-44 rounded-2xl bg-white p-2.5 shadow-2xl border border-hairline animate-in fade-in zoom-in-95 duration-150">
                <div className="grid grid-cols-4 gap-2">
                  {COLORS.map((c) => (
                    <button
                      ...
                      title={c.label}
                      onClick={() => {
                        setAndSaveColor(c.hex);
                        setIsColorMenuOpen(false);
                      }}
                      className={`size-7 rounded-full border transition-transform ${
                        strokeColor === c.hex ? "scale-115 ring-2 ring-navy ring-offset-1" : "hover:scale-110 border-black/10"
                      }`}
      ...
      {/* Canvas */}
      <div
        className={`relative flex-1 min-h-0 overflow-hidden ${isDark ? "bg-[#020617]" : "bg-white"}`}
