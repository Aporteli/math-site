...
function applySource(
  problem: BankProblem,
  next: EditSource,
): Pick<BankProblem, "source" | "templateId"> {
  if (next === "verified") {
    return {
      source:
        problem.source === "bank" || problem.source === "custom"
          ? "ai"
          : problem.source,
  ...
  if (next === "generated") {
    return {
      source: "generated",
      templateId:
        problem.templateId === "ai-plain" || problem.templateId === "ai-verified"
          ? "generated"
          : problem.templateId,
    };
  }
...
function initialTaxonomy(
  ...
  let branchId = problem.branchId && byId.has(problem.branchId)
    ? problem.branchId
    : ("all" as const);
  let topicNodeId =
    problem.topicNodeId && byId.has(problem.topicNodeId)
      ? problem.topicNodeId
      : ("all" as const);
  let subtopicId =
    problem.subtopicId && byId.has(problem.subtopicId)
      ? problem.subtopicId
      : ("all" as const);
  let conceptId =
    problem.conceptId && byId.has(problem.conceptId)
      ? problem.conceptId
      : ("all" as const);
  ...
  if (topicNodeId === "all" && problem.topic) {
    const match = nodes.find(
      (node) => node.level === "topic" && node.slug === problem.topic,
    );
    if (match) {
...
export function EditProblemModal({
  ...
  const [taxonomy, setTaxonomy] = useState(() =>
    initialTaxonomy(problem, taxonomyNodes),
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setTaxonomy((current) => {
        const next = initialTaxonomy(
          {
            ...
            topicNodeId:
              current.topicNodeId === "all" ? undefined : current.topicNodeId,
            subtopicId:
              current.subtopicId === "all" ? undefined : current.subtopicId,
            conceptId:
              current.conceptId === "all" ? undefined : current.conceptId,
          },
          fresh,
        );
        ...
        return {
          branchId:
            current.branchId !== "all" && ids.has(current.branchId)
              ? current.branchId
              : next.branchId,
          topicNodeId:
            current.topicNodeId !== "all" && ids.has(current.topicNodeId)
              ? current.topicNodeId
              : next.topicNodeId,
          subtopicId:
            current.subtopicId !== "all" && ids.has(current.subtopicId)
              ? current.subtopicId
              : next.subtopicId,
          conceptId:
            current.conceptId !== "all" && ids.has(current.conceptId)
              ? current.conceptId
              : next.conceptId,
        };
      });
  ...
  const branchOptions = useMemo(
    () => childrenOf(nodes, null, "branch").map((n) => n.id),
    [nodes],
  );
  ...
  async function onSubmit() {
    ...
    const topicNode =
      taxonomy.topicNodeId !== "all"
        ? nodes.find((node) => node.id === taxonomy.topicNodeId)
        : undefined;
    ...
    const next: BankProblem = {
      ...
      solutionTex,
      branchId:
        taxonomy.branchId === "all" ? undefined : taxonomy.branchId,
      topicNodeId:
        taxonomy.topicNodeId === "all" ? undefined : taxonomy.topicNodeId,
      subtopicId:
        taxonomy.subtopicId === "all" ? undefined : taxonomy.subtopicId,
      conceptId:
        taxonomy.conceptId === "all" ? undefined : taxonomy.conceptId,
    };
      ...
      <div className="pointer-events-none relative z-10 flex h-full items-center justify-center p-4 sm:p-6">
        ...
        >
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div>
                ...
                <SelectMenu
                  ...
                  value={year}
                  onChange={(value) =>
                    setYear(value === "" ? "" : (value as ProblemYear))
                  }
                  options={[
          ...
          <div className="flex shrink-0 flex-col gap-2 border-t border-hairline px-4 py-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end sm:px-5">
            {notice ? (
              <p className="w-full text-sm text-brass-strong sm:me-auto sm:w-auto">
                {notice}
              </p>
            ) : null}
            <button
