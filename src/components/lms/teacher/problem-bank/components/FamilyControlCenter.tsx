...
function familyErrorText(copy: ProblemBankCopy['familyCenter'], error: string) {
  switch (error) {
    case 'unauthorized':
...
function upsertFamily(families: SavedProblemFamily[], family: SavedProblemFamily) {
...
export function FamilyControlCenter({
  ...
  const familyGenerateLabels = (() => {
    ...
    try {
      const labels = collectTemplateGenerateLabels(JSON.parse(jsonText) as unknown);
      return {
  ...
  const allVariantsSelected = listedVariants.length > 0 && selectedVariantIndexes.length === listedVariants.length;
  ...
  async function generateProblems() {
    ...
    try {
      if (selected && !dirty) {
        ...
        setNotice(replaceTokens(center.generated, { count: result.problems.length }));
        return;
      }
      ...
      const stamped = selected ? stampFamilySource(created, selected) : created;
      if (stamped.length === 0) {
        const status = classifyTemplateGenerateFilter(read.template, {
          difficulty: genDifficulty === 'any' ? undefined : genDifficulty,
          year: genYear === 'any' ? undefined : genYear,
        });
        setNotice(status === 'no_match' ? center.noMatchGenerate : center.emptyGenerate);
        return;
      }
      onCreated(stamped);
  ...
  async function removeSelected() {
    ...
    try {
      ...
      const parent = parentId ? (next.find((family) => family.id === parentId) ?? null) : null;
  ...
  async function submitVariantPaste() {
    ...
    const saved = await persistFamilyJson(result.json, replaceTokens(center.addVariantAdded, { count: result.added }));
    if (!saved) return;
    setShowVariantPaste(false);
    setVariantPaste('');
    setVariantAuditIssues([]);
  ...
  async function relabelVariantAt(index: number, labels: { year?: ProblemYear; difficulty?: ProblemDifficulty }) {
    setNotice(null);
  ...
  function toggleSelectAllVariants() {
    setConfirmBulkDelete(false);
    setSelectedVariantIndexes((current) =>
      current.length === listedVariants.length ? [] : listedVariants.map((_, index) => index),
    );
  }
  ...
  async function removeSelectedVariants() {
    ...
    const result = removeVariantsFromFamilyJson(jsonText, selectedVariantIndexes);
    if (!result.ok) {
      setNotice(center.invalidJson);
      setConfirmBulkDelete(false);
      return;
  ...
  const previewBlocked = Boolean(
    preview && 'diagnosis' in preview && preview.diagnosis && preview.diagnosis.hint !== 'empty',
  );
  ...
  return (
    <section className={`${panelClass} mt-6`} aria-labelledby="families-heading">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-hairline pb-4">
        <h2 id="families-heading" className="text-lg font-semibold tracking-tight text-ink">
          {center.title}
        </h2>
        <button
          type="button"
                ...
                return (
                  <li key={family.id}>
                    <button
                      ...
                      <span className="shrink-0 tabular-nums text-[11px] text-muted">{family.variantCount}</span>
                    </button>
                  </li>
                );
              })}
          ...
          {!selected && !jsonText.trim() ? (
            ...
            <>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted">
                      ...
                      {replaceTokens(center.updated, {
                        date: new Date(selected.updatedAt).toLocaleDateString(locale),
                      })}
                    </p>
                  </div>
                  <button
                      ...
                      <>
                        ...
                        <ul className="max-h-72 space-y-1 overflow-y-auto px-2 pb-2">
                          {listedVariants.map((variant, index) => {
                            const slug = variant.id?.trim() || `task-${index + 1}`;
                            const displayNumber = index + 1;
                            const familyYears = familyRead?.ok ? familyRead.template.years : [];
                            const familyDifficulties = familyRead?.ok ? familyRead.template.difficulties : [];
                            const yearValue = variant.years?.[0] ?? familyYears[0] ?? PROBLEM_YEARS[0];
                            const difficultyValue =
                              variant.difficulties?.[0] ?? familyDifficulties[0] ?? PROBLEM_DIFFICULTIES[0];
                            ...
                            return (
                              <li
                                ...
                                <button
                                  ...
                                  <Trash2 className="size-3.5" aria-hidden="true" />
                                  {confirmVariantIndex === index ? center.confirmRemoveVariant : center.removeVariant}
                                </button>
                              </li>
                            );
                          })}
              ...
              {showVariantPaste ? (
                <div className="rounded-2xl border border-navy/15 bg-navy-tint/30 p-4">
                  ...
                  {variantPasteIssues.length > 0 ? (
                    <div className="mt-3">
                      <AuditPanel copy={copy.importFamily} issues={variantPasteIssues} />
                    </div>
                  ) : variantAuditIssues.length > 0 ? (
                    <div className="mt-3">
                      <AuditPanel copy={copy.importFamily} issues={variantAuditIssues} />
                    </div>
                  ) : null}
              ...
              {preview && 'problem' in preview && preview.problem ? (
                ...
                <DiagnosisNote copy={copy.importFamily} diagnosis={preview.diagnosis} />
              ) : null}

              {notice ? (
                <p className="cursor-pointer text-sm text-brass-strong" onClick={() => setNotice(null)}>
                  {notice}
                </p>
              ) : null}
              ...
              <div className="rounded-2xl border border-hairline bg-paper p-4">
                <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <SelectMenu
                      ...
                      options={[
                        ...
                        ...PROBLEM_DIFFICULTIES.map((item) => {
                          const marked = familyGenerateLabels.difficulties.has(item);
                          return {
                            value: item,
                  ...
                  <div>
                    <label className="block text-sm font-medium text-ink" htmlFor={`${jsonId}-year`}>
                      {copy.generate.year}
                    </label>
                    <SelectMenu
                      id={`${jsonId}-year`}
