...
export function QuadraticCalculator({ locale, copy, title, description }: QuadraticCalculatorProps) {
  ...
  const [exportPack, setExportPack] = useState<{ markdown: string; latex: string; summary: string } | null>(null);
        ...
        <div className="space-y-4">
          <section className={panelClass}>
            <form onSubmit={handleSolve} className="space-y-3">
              <label className="block text-xs font-semibold text-muted">განტოლება</label>
              <input
                ref={inputRef}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <label className="text-xs text-muted">ცვლადი</label>
                <select
                ...
                <button
                  type="submit"
                  className="ml-auto inline-flex items-center gap-1.5 rounded-xl bg-navy px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-navy/90 dark:bg-sky-600">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-xl border border-hairline px-3 py-2 text-sm text-muted hover:text-ink dark:border-slate-700">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {PRESETS.map((p) => (
                <button
                  ...
                  className="rounded-lg border border-hairline px-2 py-1 text-[11px] text-muted hover:border-navy/30 hover:text-ink dark:border-slate-700">
                  {p.label}
                </button>
              ))}
            </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {g.keys.map((k) => (
                        <button
                          ...
                          type="button"
                          onClick={() => insertKey(k)}
                          className="min-w-[2rem] rounded-lg border border-hairline bg-paper/50 px-2 py-1.5 font-mono text-xs hover:bg-navy/5 dark:border-slate-700 dark:bg-slate-800">
              ...
              <div className="space-y-2">
                {activeSteps.map((st, idx) => (
                  <div key={idx} className="text-xs">
                    ...
                    {st.latex ? <KatexPreview tex={st.latex} /> : st.explanation}
                  </div>
                ))}
              </div>
            </section>
                    ...
                    {analysisRows.map((row, i) => (
                      <tr key={i} className="border-b border-hairline last:border-0 dark:border-slate-800">
                        ...
                        <td className="py-2">
                          ...
                          {row.note && <p className="mt-0.5 text-[10px] text-muted">{row.note}</p>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
