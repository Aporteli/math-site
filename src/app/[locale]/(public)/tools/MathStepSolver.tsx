    ...
    return [
      {
        ...
        phase1Text: "მონიშნულია ძირითადი წევრები და მათი ნიშნები.",
        phase2Text: "მივიღეთ სუფთა მნიშვნელობები: a = m-1, b = -2(m+1), c = m-2.",
        tokensPhase1: [
      ...
      },
      {
        ...
        phase1Text: "ავირჩიეთ დისკრიმინანტის გამოსათვლელი მოდელი.",
        phase2Text: "მსგავსი წევრების შეკვეცით მივიღეთ: D = 20m - 4.",
        tokensPhase1: [
      ...
      },
      {
        ...
        phase1Text: "შევადგინეთ უტოლობა 20m - 4 ≥ 0.",
        phase2Text: "საბოლოო შეზღუდვა: m ≥ 1/5 (სადაც m ≠ 1).",
        tokensPhase1: [
    ...
    return [
      {
        ...
        phase1Text: "მონიშნულია ფუძე და ხარისხის მაჩვენებელი.",
        phase2Text: "მივიღეთ ორი იდენტური მამრავლი.",
        tokensPhase1: [
      ...
      },
      {
        ...
        phase1Text: "მონიშნულია გადასამრავლებელი მამრავლები.",
        phase2Text: "დაფიქსირდა სრული გაშლილი ჯამი.",
        tokensPhase1: [],
        tokensPhase2: []
      },
      {
        ...
        phase1Text: "მონიშნულია მსგავსი წევრები ab და ba.",
        phase2Text: "საბოლოო ფორმულა წარმატებით დაფიქსირდა.",
        tokensPhase1: [
  ...
  return [
    {
      ...
      phase1Text: `მონიშნულია ნიშანი ${bSign} და რიცხვი ${bAbs}.`,
      phase2Text: "რბილად ჩაჯდა თავის ახალ ადგილას.",
      tokensPhase1: [
    ...
    },
    {
      ...
      phase1Text: "მონიშნულია შესაკრები რიცხვები.",
      phase2Text: "შედეგი დაფიქსირდა პოზიციაზე.",
      tokensPhase1: [
    ...
    },
    {
      ...
      phase1Text: `მონიშნულია კოეფიციენტი ${a}.`,
      phase2Text: "წილადი წარმატებით ჩაჯდა.",
      tokensPhase1: [
    ...
    },
    {
      ...
      phase1Text: "მონიშნულია მიღებული წილადი.",
      phase2Text: "ამონახსნი სრულად დაფიქსირდა.",
      tokensPhase1: [
...
export function MathStepSolver() {
  ...
  useEffect(() => {
    ...
    const initialPositions: Record<string, TokenPosition> = {};
    activeTokens.forEach((token, index) => {
      initialPositions[token.id] = getDefaultTokenPosition(index);
    });
  ...
  const handleDuplicateToken = (token: EquationToken, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    const basePos = tokenPositions[token.id] ?? { x: 20, y: 20 };
    setTokenPositions((prev) => ({
      ...prev,
      [newToken.id]: { x: basePos.x + elementWidth + 10, y: basePos.y }
    }));
    
    setSelectedTokenId(newToken.id);
    setCustomTokenText(newToken.text);
  };
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-4 w-full min-h-0">
            {!isManualPlayground ? (
              ...
              <>
                ...
                {isFullscreen && <div className="w-full min-h-[400px]" aria-hidden="true" />}

                <div
                  className={
                    isFullscreen
                      ? "fixed inset-4 sm:inset-10 z-50 flex flex-col bg-white rounded-3xl shadow-2xl overflow-hidden border border-hairline transition-all duration-300"
                      : "w-full space-y-6"
                  }
                >
                  <div className={`border-rose-200 bg-rose-50/50 p-4 flex flex-col transition-all ${isFullscreen ? 'flex-1 min-h-0' : 'rounded-2xl border space-y-3'}`}>
                    <div className="flex flex-wrap items-center justify-between gap-2 text-rose-800 text-xs font-bold">
                      <span className="flex items-center gap-1.5">
                        ...
                        <span className="hidden sm:inline">გადაათრიეთ სიმბოლოები, ან ორჯერ დააკლიკეთ დასაკოპირებლად:</span>
                        <span className="sm:hidden">სამოძრაო დაფა:</span>
                      </span>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={handleAddToken}
                          className="inline-flex items-center gap-1 bg-rose-600 text-white px-3 py-1.5 rounded-lg text-xs shadow-xs hover:bg-rose-700 transition"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 bg-white text-rose-700 border border-rose-300 px-3 py-1.5 rounded-lg text-xs shadow-xs hover:bg-rose-50 transition"
                        >
                          {isFullscreen ? (
                            <><Minimize className="size-3.5" /> <span className="hidden sm:inline">შეკუმშვა</span></>
                          ) : (
                            <><Maximize className="size-3.5" /> <span className="hidden sm:inline">მთელ ეკრანზე</span></>
                          )}
                        </button>
                      </div>
                    </div>
                    ...
                    <div
                      key={isFullscreen ? "fullscreen-board" : "inline-board"}
                      ref={playgroundRef}
                      onPointerDown={(e) => {
                      ...
                      className={`relative overflow-hidden rounded-xl border border-hairline bg-white shadow-xs touch-none ${
                      ...
                      {playgroundTokens.map((token) => {
                        ...
                        return (
                          <motion.div
                            ...
                            dragElastic={0}
                            dragMomentum={false}
                            initial={{ x: basePos.x, y: basePos.y }}
                            animate={{ x: basePos.x, y: basePos.y }}
                            transition={{ duration: 0 }}
                            onDragEnd={(_, info) => {
                              setTokenPositions((prev) => {
                                const currentPosition = prev[token.id] ?? { x: 20, y: 20 };
                                return {
                                  ...prev,
                                  [token.id]: {
                            ...
                            className="absolute top-0 left-0 group cursor-grab active:cursor-grabbing touch-none select-none z-10"
                          >
                            <div
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className={`font-mono font-black px-3.5 py-2.5 rounded-xl border transition-colors cursor-pointer select-none ${
                                isSelected
                            ...
                            <button
                              ...
                              className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition shadow-xs z-20 hover:bg-red-600"
                              title="წაშლა"
                            >
                  ...
                  {selectedTokenId ? (
                    <div className={`border-hairline bg-paper p-4 space-y-3 ${isFullscreen ? 'border-t shrink-0' : 'rounded-2xl border'}`}>
                      ...
                      <form key={selectedTokenId} onSubmit={handleUpdateCustomToken} className="flex flex-wrap gap-2">
                        <input
                          type="text"
                    ...
                    <div className={`text-center py-6 bg-paper-deep/50 ${isFullscreen ? 'border-t border-hairline shrink-0' : 'rounded-2xl border border-dashed border-hairline'}`}>
            ...
            {isFinalStep && phase === 2 && !isManualPlayground ? (
              ...
              >
                {isManualPlayground
                  ...
                  : phase === 1 && step.isTrajectoryExpansion
                  ? "გადამრავლება →"
                  : phase === 1
                  ? "გარდაქმნა →"
