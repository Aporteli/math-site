...
function formatStandardLatex(
  ...
  if (!isZero(b)) {
    ...
    if (b.n === b.d) parts.push(parts.length ? `+ ${variable}` : variable);
      ...
      parts.push(parts.length ? `+ ${bt}${variable}` : `${bt}${variable}`);
...
export function generateDiscriminantSteps(
  sol: QuadraticSolution,
): MethodSolution {
  ...
  if (cmp(D, ZERO) > 0) {
    steps.push({
      ...
      explanation:
        "რადგან $D > 0$, განტოლებას აქვს ორი განსხვავებული ნამდვილი ფესვი.",
      latex: `D = ${toFraction(D)} > 0`,
    });
  } else if (cmp(D, ZERO) === 0) {
    steps.push({
      title: "ნაბიჯი 4: ფესვების ბუნება",
      explanation:
        "რადგან $D = 0$, განტოლებას აქვს ერთი (ორმაგი) ნამდვილი ფესვი.",
      latex: `D = 0`,
    });
  } else {
    steps.push({
      title: "ნაბიჯი 4: ფესვების ბუნება",
      explanation:
        "რადგან $D < 0$, განტოლებას აქვს ორი კომპლექსური შეუღლებული ფესვი.",
      latex: `D = ${toFraction(D)} < 0`,
    });
  }
  ...
  if (
    ...
    const rootLatex = roots
      .map((r, i) => `${variable}_{${i + 1}} = ${r.exact}`)
      .join(", \\quad ");
    steps.push({
...
export function generateCompleteSquareSteps(
  sol: QuadraticSolution,
): MethodSolution {
...
export function generateFactoringSteps(
  sol: QuadraticSolution,
): MethodSolution | null {
  ...
  steps.push({
    ...
    explanation:
      "ნამრავლი ნულის ტოლია მაშინ და მხოლოდ მაშინ, როცა ერთ-ერთი მამრავალი მაინც ნულია.",
    latex: roots
      .map((r) => `${variable} - (${r.exact}) = 0`)
      .join(" \\quad \\text{ან} \\quad "),
  });
...
export function generateAllMethods(sol: QuadraticSolution): MethodSolution[] {
  if (
    sol.classification === "linear" ||
    sol.classification === "identity" ||
    sol.classification === "inconsistent" ||
    sol.classification === "not-quadratic"
  ) {
