...
import { PROBLEM_DIFFICULTIES, PROBLEM_INSTRUCTIONS, PROBLEM_TOPICS, PROBLEM_YEARS } from './types';
...
export const generateDiverseProblemsSchema = z
  .object({
    ...
    history: z.array(aiChatTurnSchema).max(20).default([]).optional(),
  })
  .transform((data) => ({
    ...data,
...
export type GenerateDiverseProblemsInput = z.infer<typeof generateDiverseProblemsSchema>;
  ...
  properties: {
    problems: {
      ...
      items: {
        ...
        required: ['kind', 'topic', 'difficulty', 'year', 'instructionId', 'promptTex', 'formula', 'variables'],
      },
    },
  },
  required: ['problems'],
  ...
  properties: {
    problems: {
      ...
      items: {
        ...
        required: ['kind', 'topic', 'difficulty', 'year', 'promptTex', 'solutionTex'],
      },
    },
  },
  required: ['problems'],
