...
const BLOCKED = /\b(constructor|prototype|window|document|Function|eval|globalThis|import|createUnit|random)\b/i;
...
export const CAS_EXTRA_FUNCTIONS = ['sind', 'cosd', 'tand', 'asind', 'acosd', 'atand', 'nCr', 'nPr'] as const;
...
const RESERVED = new Set(['i', ...CAS_CONSTANTS, ...ALLOWED_FUNCTIONS]);
 ...
 */
export type CasEvalOptions = {
  /** Template params/derived may exceed the formula-verification magnitude cap. */
  allowLargeScope?: boolean;
  /** Keep irrationals/decimals in template derived chains (no snapClean). */
  skipSnapClean?: boolean;
};

const TEMPLATE_EVAL_OPTS: CasEvalOptions = {
    ...
    const variables =
      names.length === 0
        ...
        : sanitizeVariables(
            ...
            { allowReserved: true, allowLargeScope: options?.allowLargeScope },
          );
...
function splitTopLevel(expr: string, seps: readonly string[]): string[] {
  ...
  for (let i = 0; i < expr.length; i += 1) {
    ...
    if (char === '(') depth += 1;
      ...
      const hit = sorted.find((sep) => expr.slice(i, i + sep.length).toLowerCase() === sep.toLowerCase());
      if (hit) {
...
function evalCompareChain(
  ...
  const parts: Array<{ kind: 'expr'; value: string } | { kind: 'op'; value: CompareOp }> = [];
  let depth = 0;
  let buf = '';
  let i = 0;
  while (i < expr.length) {
  ...
  const exprs = parts.filter((part): part is { kind: 'expr'; value: string } => part.kind === 'expr');
  const ops = parts.filter((part): part is { kind: 'op'; value: CompareOp } => part.kind === 'op');
  if (ops.length === 0) return evaluateCas(expr, scope, options);
  ...
  for (const part of exprs) {
    const result = evaluateCas(part.value, scope, options);
...
function evalAndExpr(
  ...
  if (parts.length <= 1) return evalCompareChain(expr, scope, options);
  for (const part of parts) {
    if (!part) return { ok: false, reason: 'unsafe' };
    const result = evalCompareChain(part, scope, options);
    if (!result.ok) return result;
...
export function evaluateTemplateExpr(
  ...
  if (orParts.length <= 1) return evalAndExpr(expr, scope, TEMPLATE_EVAL_OPTS);

  for (const part of orParts) {
    if (!part) return { ok: false, reason: 'unsafe' };
    const result = evalAndExpr(part, scope, TEMPLATE_EVAL_OPTS);
    if (!result.ok) return result;
...
export function sanitizeVariables(
  ...
  options?: { allowReserved?: boolean; allowLargeScope?: boolean },
  ...
  for (const entry of entries) {
    ...
    if (!options?.allowLargeScope && Math.abs(entry.value) > VARIABLE_ABS_MAX) {
      throw new Error('value');
    }
...
function assertSafeAst(node: MathNode, varNames: Set<string>) {
  node.traverse((child: MathNode) => {
    ...
    if (symbol && !CONSTANT_SET.has(symbol) && !varNames.has(symbol) && !ALLOWED_FUNCTIONS.has(symbol)) {
      throw new Error('sym');
    }
  });
}
...
export function verifyFormula(rawFormula: string, rawVariables: { name: string; value: number }[]): CasOk | CasFail {
  try {
    const formula = sanitizeFormula(rawFormula);
