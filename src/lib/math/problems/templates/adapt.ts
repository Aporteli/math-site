...
import { buildWorkRateFamily, relayAlphaHours, slotWorkRateStory, workRateHoursFromEquation } from './work-rate';
...
const TOPIC_HINTS: [RegExp, ProblemTopic][] = [
  ...
  [/geometry|triangle|pythag|circle|trapez|ტრაპეც|მართკუთხ|გეომეტ/i, 'geometry'],
  [/percent/i, 'percent'],
  ...
  [/algebra|polynomial|vieta|quadratic|cubic|linear|sequence|series|progression|binomial/i, 'algebra'],
...
function repairImportedTexSlots(text: string, names: Set<string>) {
  ...
  let out = text;
  out = out.replace(/\^\{\{(\w+)\}(\s*-\s*3r)\}/g, (full, name: string, rest: string) =>
    has(name) ? `^{{{${name}}}${rest}}` : full,
  );
  out = out.replace(/\^\{\{(\w+)\}-/g, (full, name: string) => (has(name) ? `^{{{${name}}}-` : full));
  out = out.replace(/\\frac\{\{(\w+)\}\}/g, (full, name: string) => (has(name) ? `\\frac{{{${name}}}}` : full));
  out = out.replace(/\\binom\{\{(\w+)\}\}\{\{(\w+)\}\}/g, (full, a: string, b: string) =>
    has(a) && has(b) ? `\\binom{{{${a}}}}{{{${b}}}}` : full,
  );
  out = out.replace(/\\binom\{\{(\w+)\}\}\{r\}/g, (full, name: string) =>
    has(name) ? `\\binom{{{${name}}}}{r}` : full,
  );
  out = out.replace(/\^\{\{(\w+)\}\}(?![\w{])/g, (full, name: string) => (has(name) ? `^{{{${name}}}}` : full));
  out = out.replace(/T_\{\{(\w+)\}\}/g, (full, name: string) => (has(name) ? `T_{{{${name}}}}` : full));
  out = out.replace(/\{\{(\w+)\}\}\^\{\{(\w+)\}\}/g, (full, a: string, b: string) =>
    has(a) && has(b) ? `{{{${a}}}}^{{{${b}}}}` : full,
  );
  out = out.replace(
    /_\{+\s*([A-Za-z][A-Za-z0-9_]{0,23})\s*\}\s*\\times\s*\{+\s*([A-Za-z][A-Za-z0-9_]{0,23})\s*\}+/g,
    (full, a: string, b: string) => (has(a) && has(b) ? `_{{{${a}}} \\times {{${b}}}}` : full),
  );
  out = out.replace(/\{\{(\w+)\}\}\{\{(\w+)\}\}/g, (full, a: string, b: string) =>
  ...
  out = out.replace(/\\cdot \{\{(\w+)\}\}\^r/g, (full, name: string) => (has(name) ? `\\cdot {{{${name}}}}^r` : full));
...
function yearsFor(_difficulty: ProblemDifficulty): ProblemYear[] {
  return [...PROBLEM_YEARS];
}
...
function yearsFromId(id: unknown): ProblemYear[] {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    const year = coerceYear(parts[i]);
    if (!year) continue;
    const prev = parts[i - 1] ?? '';
    if (/^v\d+$/i.test(prev)) continue;
    if (/^\d{4}$/.test(prev)) continue;
    return [year];
  }
  return [];
}
...
function mapInstruction(value: unknown, prompt: string, hasNumericAnswer: boolean): ProblemInstructionId {
  if (typeof value === 'string' && (PROBLEM_INSTRUCTIONS as readonly string[]).includes(value)) {
...
function mapParam(spec: unknown): Record<string, unknown> | null {
  ...
  const looksInt = type === 'integer' || type === 'int' || type === 'number' || (min != null && max != null);
  if (!looksInt || (min == null && max == null)) return null;

  const lo = min ?? max ?? 1;
  const hi = max ?? min ?? lo;
  const exclude = Array.isArray(rec.exclude)
    ? rec.exclude.filter((item): item is number => typeof item === 'number' && Number.isInteger(item)).slice(0, 24)
    : [];
  return {
    int: [Math.min(lo, hi), Math.max(lo, hi)],
    ...(rec.nonzero === true || lo > 0 || hi < 0 || exclude.includes(0) ? { nonzero: true } : {}),
...
function mapParams(value: unknown): Record<string, unknown> {
  ...
  for (const [name, spec] of Object.entries(rec)) {
    if (!/^[A-Za-z][A-Za-z0-9_]{0,23}$/.test(name)) continue;
...
function mapDerived(value: unknown): Record<string, string> {
  ...
  for (const [name, expr] of Object.entries(rec)) {
    ...
    const trimmed = expr.trim().slice(0, 320);
...
function mapConstraints(value: unknown): string[] {
  ...
  for (const item of value) {
    ...
    if (rewritten) out.push(rewriteCasExpr(rewritten).slice(0, 320));
    if (out.length >= 24) break;
  }
  return out;
}

function inferredDistinctConstraints(params: Record<string, unknown>, text: string): string[] {
...
function nestedString(rec: Record<string, unknown>, key: string): string {
  ...
  if (!child) return '';
  for (const name of ['combined_equation', 'simplified_equation', 'simplified_equation_latex', 'equation']) {
...
function intBand(value: number, span: number, min = 1): { int: [number, number]; nonzero: true } {
...
function adaptBinomialFormulaDoc(rec: Record<string, unknown>) {
  const latex =
    ...
    '';
  const name = typeof rec.formula_name === 'string' ? rec.formula_name.trim() : '';
  const hay = `${name} ${latex}`;
  if (!/binom|binomial|T_\s*\{\s*r\s*\+\s*1\s*\}/i.test(hay)) return null;

  const components = asRecord(rec.components);
  const nFromComp = typeof components?.n === 'number' && Number.isInteger(components.n) ? components.n : null;
...
function adaptWorkRateFamily(rec: Record<string, unknown>) {
  const equation =
    (typeof rec.combined_equation === 'string' ? rec.combined_equation : '') || nestedString(rec, 'mathematical_model');
  const hours = workRateHoursFromEquation(equation);
  if (!hours) return null;

  const statement =
    typeof rec.problem_statement === 'string' && rec.problem_statement.trim() ? rec.problem_statement.trim() : '';
  const prompt = slotWorkRateStory(statement, hours.d, hours.h1, hours.h2);
  const id = typeof rec.id === 'string' && rec.id.trim() ? rec.id.trim().slice(0, 64) : 'two-cluster-work';
...
function looksLikeTaskDoc(rec: Record<string, unknown>) {
  return (
    ...
    (asRecord(rec.operations) != null || asRecord(rec.answers) != null || typeof rec.set === 'string')
  );
}
...
function topicHaystack(rec: Record<string, unknown>) {
  return [rec.topic, rec.title, rec.set, rec.field, JSON.stringify(rec.operations ?? '')]
...
function adaptTaskListFamily(rec: Record<string, unknown>) {
  ...
  const axiom = /აქსიომ|axiom|ვექტორულ სივრც|vector_addition|scalar_multiplication/i.test(hay);
  ...
  return {
    ...
    topic: mapTopic(hay),
    difficulties: axiom ? (['hard'] as const) : diffs.length > 0 ? diffs : [difficulty],
    ...
    variants: uniquifyVariantIds(
      templates.flatMap((item) =>
        item.variants.map((variant) => ({
          ...
          difficulties: variant.difficulties?.length ? variant.difficulties : item.difficulties,
        })),
      ),
...
function packagedProblemList(rec: Record<string, unknown>): { key: string; list: unknown[] } | null {
  const keys = ['vector_space_problems', 'problems', 'items', 'cards', 'families', 'examples'];
  for (const key of keys) {
    const value = rec[key];
    if (Array.isArray(value) && value.length > 0 && value.every((item) => asRecord(item) != null)) {
  ...
  if (looksLikeTaskDoc(first) || typeof first.prompt === 'string' || Array.isArray(first.variants)) {
    return { key, list };
  }
  return null;
}
...
function isAuthorVariantCard(rec: Record<string, unknown>) {
  if (Array.isArray(rec.variants)) return false;
  return (
    typeof rec.prompt === 'string' ||
    typeof rec.prompt_latex === 'string' ||
    typeof rec.promptTex === 'string' ||
    typeof rec.problem_statement === 'string' ||
    typeof rec.statement === 'string' ||
    typeof rec.text === 'string' ||
    looksLikeTaskDoc(rec)
  );
}

function adaptAuthorVariantCard(rec: Record<string, unknown>, index: number): TemplateVariant | null {
  ...
  const steps = stringList(rec.solutionSteps ?? rec.solution_steps ?? rec.steps).map((step) =>
    repairImportedTexSlots(expandSlots(step, names), names).slice(0, 1600),
  );
  if (steps.length === 0) {
    steps.push(...answersToSteps(rec.answers ?? rec.correct_answer ?? rec.answer).map((step) => step.slice(0, 1600)));
  }
  ...
  if (answer && !steps.some((step) => step.includes(answer))) {
    steps.push(answer.slice(0, 1600));
  }
  const clippedSteps = steps.filter((step) => step.trim()).slice(0, 24);
  ...
  const formulaLooksLikeCas = formulaText.length > 0 && !/\\|binom|frac|cdot/.test(formulaText);
  const explicitFormula = formulaLooksLikeCas ? formulaText : formulaFromAnswer(answer, derived, params);
  ...
  return {
    id,
    params: params as TemplateVariant['params'],
    derived,
    constraints,
    prompt: prompt.slice(0, 4000),
    ...(years.length > 0 ? { years } : {}),
    ...(listedDifficulties.length > 0 ? { difficulties: listedDifficulties } : {}),
    ...(explicitFormula ? { formula: explicitFormula.slice(0, 320) } : {}),
    ...(clippedSteps.length > 0
      ? { solutionSteps: clippedSteps }
      : { solution: answer || prompt.slice(0, 4000) }),
  };
}

function adaptAuthorVariantList(docs: Record<string, unknown>[]) {
  const variants = docs.flatMap((doc, index) => {
    const variant = adaptAuthorVariantCard(doc, index);
    return variant ? [variant] : [];
  });
  if (variants.length === 0) return null;

  const first = docs[0]!;
  const listedDifficulties = uniqueList(docs.flatMap((doc) => difficultiesFromRecord(doc)));
  const difficulty = listedDifficulties[0] ?? mapDifficulty(first.difficulty);
  const years = uniqueList(docs.flatMap((doc) => yearsFromRecord(doc)));
  const prompt = variants[0]?.prompt ?? '';

  return {
    id: familyIdFrom(first, 'imported-family'),
    topic: mapTopic(docs.map((doc) => topicHaystack(doc) || String(doc.topic ?? '')).join(' ')),
    difficulties: listedDifficulties.length > 0 ? listedDifficulties : [difficulty],
    years: years.length > 0 ? years : yearsFor(difficulty),
    instructionId: mapInstruction(first.instructionId, prompt, false),
    variants: uniquifyVariantIds(variants),
  };
}

/** Normalize near-engine AI/import JSON so Zod enums and lengths pass. */
function sanitizeEngineShapedFamily(rec: Record<string, unknown>): unknown | null {
  if (!Array.isArray(rec.variants) || rec.variants.length === 0) return null;

  const variants = rec.variants.flatMap((item, index) => {
    const card = asRecord(item);
    if (!card) return [];
    const merged: Record<string, unknown> = {
      ...card,
      years: card.years ?? card.year ?? rec.years ?? rec.year,
      difficulties: card.difficulties ?? card.difficulty ?? rec.difficulties ?? rec.difficulty,
      topic: card.topic ?? rec.topic,
    };
    const variant = adaptAuthorVariantCard(merged, index);
    return variant ? [variant] : [];
  });
  if (variants.length === 0) return null;

  const listedDifficulties = uniqueList([
    ...difficultiesFromRecord(rec),
    ...variants.flatMap((variant) => variant.difficulties ?? []),
  ]);
  const difficulty = listedDifficulties[0] ?? mapDifficulty(rec.difficulty);
  const years = uniqueList([
    ...yearsFromRecord(rec),
    ...variants.flatMap((variant) => variant.years ?? []),
  ]);
  const prompt = variants[0]?.prompt ?? '';

  return {
    id: familyIdFrom(rec, 'imported-family'),
    topic: mapTopic(topicHaystack(rec) || String(rec.topic ?? '') || prompt),
    difficulties: listedDifficulties.length > 0 ? listedDifficulties : [difficulty],
    years: years.length > 0 ? years : yearsFor(difficulty),
    instructionId: mapInstruction(rec.instructionId, prompt, false),
    variants: uniquifyVariantIds(variants),
  };
}
...
export function adaptExternalTemplate(raw: unknown): unknown {
  if (Array.isArray(raw)) {
    ...
    if (docs.every(isAuthorVariantCard)) {
      const list = adaptAuthorVariantList(docs);
      if (list) return list;
    }
  ...
  if (alreadyEngine) {
    const sanitized = sanitizeEngineShapedFamily(rec);
  ...
  const variant = adaptAuthorVariantCard(rec, 0);
  if (!variant) return rec;

  const listedDifficulties = difficultiesFromRecord(rec);
  const difficulty = listedDifficulties[0] ?? mapDifficulty(rec.difficulty);
  const years = yearsFromRecord(rec);
  const familyDifficulties = listedDifficulties.length > 0 ? listedDifficulties : [difficulty];
  ...
  return {
    ...
    years: familyYears,
    instructionId: mapInstruction(rec.instructionId, variant.prompt, Boolean(answer)),
    variants: [variant],
  };
}
